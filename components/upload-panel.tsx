...
function PreviewDialog({ upload, onClose }: { upload: Upload | null; onClose: () => void }) {
  const isVideo = upload?.file_type.startsWith("video/");
  const [copied, setCopied] = useState(false);

  const handleCopy = () => {
    if (!upload) return;
    navigator.clipboard.writeText(upload.file_url);
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
  };

  return (
    <Dialog open={!!upload} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="min-w-[900px] max-w-[90vw] p-0 overflow-hidden gap-0 bg-muted">
        <DialogHeader className="px-4 py-3 border-b bg-muted shrink-0">
          <DialogTitle className="text-sm font-semibold truncate">{upload?.original_name}</DialogTitle>
        </DialogHeader>
        <div className="w-full h-[506px] bg-sidebar flex items-center justify-center">
          {upload && (isVideo ? (
            <video
              src={upload.file_url}
              controls
        ...
        <div className="px-4 py-3 flex items-center justify-between gap-4 border-t">
          <div className="flex items-center gap-4">
            <div className="flex flex-col gap-0.5">
              ...
              <span className="text-xs font-medium">{upload?.file_type ?? "—"}</span>
            </div>
            <div className="w-px h-6 bg-border" />
            <div className="flex flex-col gap-0.5">
              <span className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Size</span>
              <span className="text-xs font-medium">{upload ? formatBytes(upload.size) : "—"}</span>
            </div>
          </div>
          <Button size="sm" variant="outline" className="h-7 text-xs font-medium gap-1.5 shrink-0" onClick={handleCopy}>
            {copied ? <><Check className="size-3" />Copied</> : "Copy link"}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
...
function UploadedThumb({
  ...
  return (
    <div
      style={style}
      className="relative overflow-hidden rounded-md border border-border bg-muted cursor-grab active:cursor-grabbing shrink-0 animate-in fade-in duration-200"
      draggable
...
export function UploadPanel() {
  ...
  useEffect(() => {
    if (uploadRefreshKey > 0) fetchUploads();
  ...
  return (
    <>
      ...
      <PreviewDialog upload={previewUpload} onClose={() => setPreviewUpload(null)} />
    </>
  );
}
